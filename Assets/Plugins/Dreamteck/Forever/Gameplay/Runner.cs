namespace Dreamteck.Forever
{
    ...
    public class Runner : MonoBehaviour
    {
        ...
        public double PeersentFromResult => _result.percent;
        ...
        public virtual void SetPercent(double percent)
        {
            ...
            _segment.Evaluate(localPercent, _result);
        ...
        public class MotionModule
        {
            ...
            public SplineSample SplineSample = new SplineSample();
            ...
            internal Vector3 GetPosition(Vector3 inputPosition)
            {
                ...
                if (useSplineSizes)
                {
                    finalOffset.x *= SplineSample.size;
                    finalOffset.y *= SplineSample.size;
                }
                if (finalOffset != Vector2.zero) position += SplineSample.right * finalOffset.x + SplineSample.up * finalOffset.y;
            ...
            {
                Vector3 resultDirection = SplineSample.forward;
                rotation = Quaternion.LookRotation(resultDirection, SplineSample.up);
            ...
            {
                if (applyScaleX) inputScale.x = baseScale.x * SplineSample.size;
                if (applyScaleY) inputScale.y = baseScale.y * SplineSample.size;
                if (applyScaleZ) inputScale.z = baseScale.z * SplineSample.size;
                return inputScale;
            }
