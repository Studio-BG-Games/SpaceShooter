...
namespace Dreamteck.Forever
{
    public class RandomPathGenerator : HighLevelPathGenerator
    {
        ...
        protected override void GeneratePoint(ref Point point, int pointIndex)
        {
            if (isFirstPoint)
            {
                ...
                if (useYaw && orientation.y == targetAngle.y)
                {
                    ...
                    turnRate.y = randomizer.NextFloat(minTurnRate.y, maxTurnRate.y);
                }
                if (usePitch && orientation.x == targetAngle.x)
                {
                    ...
                    turnRate.x = randomizer.NextFloat(minTurnRate.x, maxTurnRate.x);
                }
                if (useRoll && orientation.z == targetAngle.z)
                {
                    ...
                    turnRate.z = randomizer.NextFloat(minTurnRate.z, maxTurnRate.z);
                }
            ...
            if (useColors) point.color = Color.Lerp(minColor.Evaluate(progress), maxColor.Evaluate(progress), randomizer.Next01());
            if (useSizes) point.size = Mathf.Lerp(minSize.Evaluate(progress), maxSize.Evaluate(progress), randomizer.Next01());
        }
        ...
        {
            float direction = randomizer.NextFloat(0, 100) > 50f ? 1f : -1f;
            float move = randomizer.NextFloat(minStep, maxStep) * direction;
            input += move;
            if (restrict)
            {
        ...
        protected override void OnPostGeneration(SplinePoint[] points)
        {
            ...
            {
                OffsetPoints(points, Vector3.Lerp(minSegmentOffset, maxSegmentOffset, randomizer.NextFloat(0f, 1f)), segmentOffsetSpace);
            }
            
            if (isNewLevel && (newLevelMinOffset != Vector3.zero || newLevelMaxOffset != Vector3.zero))
            {
                OffsetPoints(points, Vector3.Lerp(newLevelMinOffset, newLevelMaxOffset, randomizer.NextFloat(0f, 1f)), levelOffsetSpace);
            }
        }
        ...
        private void InitAngles()
        {
            ...
            turnRate = new Vector3(randomizer.NextFloat(minTurnRate.x, maxTurnRate.x), randomizer.NextFloat(minTurnRate.y, maxTurnRate.y), randomizer.NextFloat(minTurnRate.z, maxTurnRate.z));
