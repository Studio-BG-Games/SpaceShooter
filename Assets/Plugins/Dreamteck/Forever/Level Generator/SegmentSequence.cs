namespace Dreamteck.Forever
{
    ...
    public class SegmentSequence
    {
        ...
        public bool IsDone()
        {
            ...
            if (_lastDefinition == null || !_lastDefinition.nested || _lastDefinition.nestedSequence.IsDone())
            {
                switch (type)
                {
                    ...
                    case Type.Shuffled: if (spawnCount == 0) return true; return currentSegmentIndex >= spawnCount;
        ...
        private int GetRandomSegmentByChance(SegmentSequence sequence, int exclude = -1)
        {
            ...
            float randomValue = randomizer.NextFloat(0f, totalChance);
            float passed = 0f;
            for (int i = 0; i < sequence.segments.Length; i++)
            {
                if (i == exclude) continue;
        ...
        private void ShuffleList<T>(IList<T> list)
        {
            ...
            {
                n--;
                int k = randomizer.NextInt(0, n + 1);
                list.Swap(k, n);
            }
        }
