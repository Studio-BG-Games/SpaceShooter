namespace Dreamteck.Forever
{
    ...
    public class SplinePath
    {
        ...
        public virtual void Project(Vector3 point, SplineSample result, double from = 0.0, double to = 1.0, EvaluateMode mode = EvaluateMode.Cached)
        {
            ...
            {
                if ((point - back).sqrMagnitude < (point - front).sqrMagnitude) SplineSample.Lerp(samples[backIndex], samples[index], backProjectDist / backLength, result);
                else SplineSample.Lerp(samples[frontIndex], samples[index], frontProjectDist / frontLength, result);
            }
            else if (backIndex < index) SplineSample.Lerp(samples[backIndex], samples[index], backProjectDist / backLength, result);
            else SplineSample.Lerp(samples[frontIndex], samples[index], frontProjectDist / frontLength, result);
        }
    }
}