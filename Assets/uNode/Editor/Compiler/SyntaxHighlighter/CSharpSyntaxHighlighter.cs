...
namespace MaxyGames.SyntaxHighlighter {
	public static class CSharpSyntaxHighlighter {
		...
		private static List<MetadataReference> GetMetadataReferences() {
			...
			if(assemblies == null) {
				assemblies = uNode.Editors.EditorReflectionUtility.GetAssemblies();
			}
		...
		public static string GetRichTextAsync(string sourceCode) {
			...
			uNode.uNodeThreadUtility.Queue(() => {
				Initialize();
			...
			uNode.uNodeThreadUtility.WaitUntilEmpty();
		...
		public static string GetRichText(string sourceCode) {
			Initialize();
			SemanticModel model;
			var root = uNode.Editors.RoslynUtility.GetSyntaxTree(sourceCode, out model);
			if(model == null) {
				//throw new Exception("Unable to analize the script");
				return null;
			}
