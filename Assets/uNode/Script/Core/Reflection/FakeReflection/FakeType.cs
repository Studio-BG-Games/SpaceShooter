...
namespace MaxyGames.uNode {
	...
	class FakeGraphType : BaseFakeGraphType<RuntimeGraphType>, ISummary, ICustomIcon {
		private readonly RuntimeGraphType graphType;

		public FakeGraphType(RuntimeGraphType target) : base(target) {
			if(target == null)
		...
		public Texture GetIcon() {
			return graphType.GetIcon();
		}

		public string GetSummary() {
			return graphType.GetSummary();
		}
	}

	class FakeGraphInterface : BaseFakeGraphType<RuntimeGraphType>, ISummary, ICustomIcon {
		private readonly RuntimeGraphInterface ifaceType;

		public FakeGraphInterface(RuntimeGraphInterface target) : base(target) {
			if(target == null)
				throw new ArgumentNullException(nameof(target));
			ifaceType = target;
		}
		...
		public Texture GetIcon() {
			return ifaceType.GetIcon();
		}

		public string GetSummary() {
			return ifaceType.GetSummary();
		}
	}

	public abstract class FakeType : RuntimeType<Type>, IFakeMember {
