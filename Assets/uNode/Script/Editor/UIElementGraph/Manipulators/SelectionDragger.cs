	...
	public class SelectionDragger : Dragger {
		...
		void OnGUI() {
			if(Event.current.modifiers.HasFlags(EventModifiers.Shift)) {
				...
				if(node != null) {
					var inputFlow = UIElementUtility.GetDefaultFlowPort(node);
					...
					if(inputFlow != null) {
						...
						if(edgeView != null) {
							...
							if(outputFlow != null) {
								var connections = new HashSet<UNodeView>();
								UIElementUtility.Nodes.GetConnectedFlowNodes(edgeView.Input.owner, connections);
								if(!connections.Contains(outputFlow.owner)) {
									...
									outputFlow = UIElementUtility.GetFinishFlowPort(node);
									HighlightElement(outputFlow.Q("connector"), new Color(0, 0.5f, 1));
									HighlightElement(edgeView.input.Q("connector"), new Color(0, 0.5f, 1));
								}
							}
						...
						if(node != null) {
							...
							if(inputFlow != null) {
								...
								if(edgeView != null) {
									...
									edgeView.Output.ChangeValue(inputFlow.GetConnection());
									if(outputFlow != null) {
										var connections = new HashSet<UNodeView>();
										UIElementUtility.Nodes.GetConnectedFlowNodes(edgeView.Input.owner, connections);
										if(!connections.Contains(outputFlow.owner)) {
											outputFlow.owner.RegisterUndo();
											outputFlow.ChangeValue(edgeView.Input.GetConnection());
										} else {
											outputFlow = UIElementUtility.GetFinishFlowPort(node);
											outputFlow.ChangeValue(edgeView.Input.GetConnection());
										}
