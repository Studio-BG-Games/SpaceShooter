	...
	public class NodeBrowser : TreeView {
		...
		public static string GetRichConstructorNames(ConstructorInfo ctor) {
			...
			for(int i = 0; i < info.Length; i++) {
				...
				if(type.IsEnum){
					mConstructur += uNodeUtility.WrapTextWithColor(type.PrettyName(false, info[i].IsOut).Split('.').Last(), uNodePreference.GetPreference().itemEnumColor, false);
				} else if(type.IsInterface) {
					mConstructur += uNodeUtility.WrapTextWithColor(type.PrettyName(false, info[i].IsOut).Split('.').Last(), uNodePreference.GetPreference().itemInterfaceColor, false);
				} else {
					mConstructur += uNodeUtility.WrapTextWithColor(type.PrettyName(false, info[i].IsOut).Split('.').Last(), uNodePreference.GetPreference().itemTypeColor, false);
				}
			...
			for(int i = 0; i < nodes.Count; ++i) {
				...
				if(lastCategoryTree != null) {
					if(IsExpanded(lastCategoryTree.id) && !isSearching || isSearching && IsMatchSearch(node.name, searchString)) {
						...
						if(node.menu != null) {
							bool isFlowNode = !node.menu.type.IsSubclassOf(typeof(ValueNode));
							childItem.icon = uNodeEditorUtility.GetTypeIcon(isFlowNode ? typeof(TypeIcons.FlowIcon) : typeof(TypeIcons.ExtensionIcon)) as Texture2D;
