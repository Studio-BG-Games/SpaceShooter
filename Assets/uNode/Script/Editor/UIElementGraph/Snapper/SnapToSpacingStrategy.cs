...
namespace MaxyGames.uNode.Editors {
	class SnapToSpacingStrategy : SnapStrategy {
		...
		public override void UpdateSnap() {
            ...
            Enabled = preference.enableSnapping && preference.spacingSnapping;
		}
		...
		bool IsSnapped(Rect snappedRect, Rect sourceRect, Orientation orientation) {
			float draggedDistance = Math.Abs(orientation == Orientation.Horizontal ? snappedRect.x - sourceRect.x : snappedRect.y - sourceRect.y);
		...
		bool IsIgnoredElement(GraphElement selectedElement, GraphElement element, Rect rectToFit) {
			if (element is Edge || !element.visible || element.IsSelected(m_GraphView) || element.layout.Overlaps(selectedElement.layout)) {
				return true;
			}
		...
		void AddReferenceRects(GraphElement selectedElement, GraphElement element, Rect rectToAdd) {
			...
			if (AreElementsSuperposed(selectedElement.layout, element.layout, Orientation.Horizontal)) {
			...
			if (AreElementsSuperposed(selectedElement.layout, element.layout, Orientation.Vertical)) {
		...
		static bool AreElementsSuperposed(Rect firstElementRect, Rect secondElementRect, Orientation orientation) {
		...
		void AddSpacingPositions(List<float> positions, Rect sourceRect, Rect firstRect, Rect secondRect, Orientation orientation) {
			if (positions != null) {
				// Position before firstRect
		...
		static SpacingLine GetSpacingLine(float maxCoordinate, float spacingLineSideLength, Vector2 startPos, Vector2 endPos, Orientation orientation) {
			// Start side's line of spacingLine
			Vector2 start = orientation == Orientation.Vertical ? new Vector2(maxCoordinate, startPos.y) : new Vector2(startPos.x, maxCoordinate);
			...
			start = orientation == Orientation.Vertical ? new Vector2(linePos, startPos.y) : new Vector2(startPos.x, linePos);
			end = orientation == Orientation.Vertical ? new Vector2(linePos, endPos.y) : new Vector2(endPos.x, linePos);
			Line lineInBetween = new Line(start, end);

			// End side's line of spacingLine
			start = orientation == Orientation.Vertical ? new Vector2(maxCoordinate, endPos.y) : new Vector2(endPos.x, maxCoordinate);
			end = endPos;
