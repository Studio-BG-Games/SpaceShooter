...
namespace MaxyGames.uNode.Editors {
	...
	public class CompletionEvaluator {
		...
		private class Data {
			...
			public IList<VariableData> variables;
			public IList<uNodeProperty> properties;
			public IList<uNodeFunction> functions;
		}
		#endregion

		public delegate NodeComponent CompletionToNodeDelegate(CompletionInfo completion, GraphEditorData editorData, Vector2 graphPosition);
		...
		public CompletionEvaluator(UnityEngine.Object unityObject, CompletionSetting completionSetting) {
			if(unityObject != null) {
				...
				if(unityObject is uNodeRoot) {
					var owner = unityObject as uNodeRoot;
					data.functions = new List<uNodeFunction>(owner.Functions);
					data.variables = new List<VariableData>(owner.Variables);
					data.properties = new List<uNodeProperty>(owner.Properties);
				} else if(unityObject is INode<uNodeRoot>) {
					var owner = (unityObject as INode<uNodeRoot>).GetOwner();
					data.owner = owner;
					data.functions = new List<uNodeFunction>(owner.Functions);
					data.variables = new List<VariableData>(owner.Variables);
					data.properties = new List<uNodeProperty>(owner.Properties);
		...
		public CompletionEvaluator(CompletionSetting setting) {
			if(setting.owner != null) {
				...
				if(setting.owner is uNodeRoot) {
					var owner = setting.owner as uNodeRoot;
					data.functions = new List<uNodeFunction>(owner.Functions);
					data.variables = new List<VariableData>(owner.Variables);
					data.properties = new List<uNodeProperty>(owner.Properties);
				} else  if(setting.owner is INode<uNodeRoot>) {
					var owner = (setting.owner as INode<uNodeRoot>).GetOwner();
					data.owner = owner;
					data.functions = new List<uNodeFunction>(owner.Functions);
					data.variables = new List<VariableData>(owner.Variables);
					data.properties = new List<uNodeProperty>(owner.Properties);
		...
		private CompletionInfo[] GetCompletions(string input, out List<CompletionInfo> memberPath) {
			if(types != null && typeMap != null) {
					...
					if((prevMember == null || !isDot && !prevMember.isDot) &&
						...
						if(data != null) {
							...
							if(setting.validCompletionKind.HasFlags(CompletionKind.uNodeVariable)) {
								if(ResolveVariable(resolveData, data.variables, data.owner)) {
									continue;
								}
							}
							//Property
							if(setting.validCompletionKind.HasFlags(CompletionKind.uNodeProperty)) {
								if(ResolveProperty(resolveData, data.properties, data.owner)) {
									continue;
								}
							}
							//Function
							if(setting.validCompletionKind.HasFlags(CompletionKind.uNodeFunction)) {
								if(ResolveFunction(resolveData, data.functions, data.owner)) {
									continue;
								}
							}
						}
										...
										switch(prevMember.keywordKind) {
											case KeywordKind.This:
												...
												if(owner != null) {
													...
													} else if(owner is INode) {
														memberInfos = (owner as INode).GetNodeOwner().GetInheritType().GetMembers();
													} else {
				...
				completions.Sort((x, y) => string.Compare(x.name, y.name, StringComparison.Ordinal));
				memberPath = members;
		...
		private static bool ResolveFunction(ResolveData resolveData, IEnumerable<uNodeFunction> functions, Object owner) {
		...
		private static bool ResolveProperty(ResolveData resolveData, IEnumerable<uNodeProperty> property, Object owner) {
		...
		private static bool ResolveVariable(ResolveData resolveData, IEnumerable<VariableData> variables, Object owner) {
		...
		public static MemberInfo[] GetCompletionMembers(CompletionInfo completion) {
			...
			} else if(completion.targetObject is VariableData) {
				memberInfos = (completion.targetObject as VariableData).Type.GetMembers();
			} else if(completion.targetObject is uNodeProperty) {
				memberInfos = (completion.targetObject as uNodeProperty).ReturnType().GetMembers();
			} else if(completion.targetObject is uNodeFunction) {
				memberInfos = (completion.targetObject as uNodeFunction).ReturnType().GetMembers();
			}
			return memberInfos;
		}
			...
			switch(keywordKind) {
				...
				case KeywordKind.This:
					if(setting.owner != null) {
						...
						} else if(setting.owner is INode) {
							return (setting.owner as INode).GetNodeOwner().GetInheritType();
						}
		...
		public static List<NodeComponent> CompletionsToGraphs(IList<CompletionInfo> completions, GraphEditorData editorData, Vector2 graphPosition) {
			List<NodeComponent> result = new List<NodeComponent>();
						...
						NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
							...
							(node) => {
								...
								if(result.Count > 0) {
									...
									if(prevNode is Node) {
										Node n = prevNode as Node;
										if(n.CanGetValue()) {
											node.target.target.instance = new MemberData(n, MemberData.TargetType.ValueNode);
										}
									}
								}
								MemberDataUtility.UpdateMultipurposeMember(node.target);
						...
						switch(c.keywordKind) {
							...
							case KeywordKind.Bool:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(false);
										result.Add(node);
									});
								break;
							case KeywordKind.Byte:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(default(byte));
										result.Add(node);
									});
								break;
							case KeywordKind.Char:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(default(char));
										result.Add(node);
									});
								break;
							case KeywordKind.Decimal:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(default(decimal));
										result.Add(node);
									});
								break;
							case KeywordKind.Double:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(default(double));
										result.Add(node);
									});
								break;
							case KeywordKind.False:
							...
							case KeywordKind.Float:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(default(float));
										result.Add(node);
									});
								break;
							case KeywordKind.Int:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(default(int));
										result.Add(node);
									});
								break;
							case KeywordKind.Long:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(default(long));
										result.Add(node);
									});
								break;
							...
							case KeywordKind.Null:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = MemberData.Null;
										result.Add(node);
									});
								break;
							case KeywordKind.Object:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(new object());
										result.Add(node);
									});
								break;
							case KeywordKind.Sbyte:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(default(sbyte));
										result.Add(node);
									});
								break;
							case KeywordKind.Short:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(default(short));
										result.Add(node);
									});
								break;
							case KeywordKind.String:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData("");
										result.Add(node);
									});
								break;
							case KeywordKind.This:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = MemberData.This(c.targetOwner);
										//node.target.target = new MemberData(c.targetOwner);
										result.Add(node);
									});
								break;
							case KeywordKind.True:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(true);
										result.Add(node);
									});
								break;
							case KeywordKind.Typeof:
								if(c.parameterCompletions.Count > 0) {
									if(c.parameterCompletions.Last().member is Type) {
										...
										graphPosition,
										(node) => {
											node.target.target = new MemberData(c.parameterCompletions.Last().member);
											result.Add(node);
										});
									}
								} else {
									...
									graphPosition,
									(node) => {
										node.target.target = new MemberData(typeof(object));
										result.Add(node);
									});
								}
								break;
							case KeywordKind.Uint:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(default(uint));
										result.Add(node);
									});
								break;
							case KeywordKind.Ulong:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(default(ulong));
										result.Add(node);
									});
								break;
							case KeywordKind.Ushort:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(default(ushort));
										result.Add(node);
									});
								break;
							case KeywordKind.Void:
								NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
									graphPosition,
									(node) => {
										node.target.target = new MemberData(typeof(void));
										result.Add(node);
									});
								break;
						}
		...
		public static MemberData CompletionsToMemberData(
			...
			if(editorData != null && memberParameters != null && memberParameters.Count > 0) {
				...
				NodeEditorUtility.AddNewNode<MultipurposeNode>(editorData,
					graphPosition,
					(node) => {
						node.target.target = new MemberData(result);
						node.target.parameters = memberParameters.ToArray();
						MemberDataUtility.UpdateMultipurposeMember(node.target);
						n = node;
					});
		...
		Vector2 graphPosition = default(Vector2)) {
			...
			if(completions != null && completions.Count > 0) {
				...
				foreach(var c in completions) {
					...
					switch(c.kind) {
								...
								switch(c.keywordKind) {
									...
									case KeywordKind.This:
										if(editorData != null && editorData.graph != null) {
											...
											return new MemberData(editorData.graph, MemberData.TargetType.SelfTarget);
										}
										return MemberData.Null;
									case KeywordKind.True:
									...
									default:
										...
										if(editorData != null) {
											...
											if(nodes != null && nodes.Count > 0) {
												return new MemberData(nodes.First(), MemberData.TargetType.ValueNode);
											}
										}
									...
									{//Find best match for method
										...
										if(methodInfo.IsGenericMethodDefinition) {
											...
											if(g.Length == genericMembers.Count) {
												methodInfo = methodInfo.MakeGenericMethod(genericMembers.Select(item => item.Get<Type>()).ToArray());
											}
										}
									}
						...
						case CompletionKind.Property:
							...
							break;
						case CompletionKind.uNodeVariable:
							targetType = (c.targetObject as VariableData).Type;
							if(startType == null) {
								startType = targetType;
							}
							memberTargetType = MemberData.TargetType.uNodeVariable;
							instanced = c.targetOwner;
							break;
						case CompletionKind.uNodeLocalVariable:
							targetType = (c.targetObject as VariableData).Type;
							if(startType == null) {
							...
							break;
						case CompletionKind.uNodeParameter:
							targetType = (c.targetObject as ParameterData).Type;
							if(startType == null) {
								startType = targetType;
							}
							...
							break;
						case CompletionKind.uNodeProperty:
							targetType = (c.targetObject as uNodeProperty).ReturnType();
							if(startType == null) {
								startType = targetType;
							}
							...
							break;
						case CompletionKind.uNodeFunction:
							targetType = (c.targetObject as uNodeFunction).ReturnType();
							if(startType == null) {
							...
							instanced = c.targetOwner;
							if(editorData != null) {
								var func = c.targetObject as uNodeFunction;
								var parameters = c.parameterCompletions;
								if(parameters != null && parameters.Count > 0) {
								...
								if(func.parameters.Length > 0) {
									...
									iData.parameters = MemberDataUtility.ParameterDataToTypeDatas(func.parameters, func.genericParameters);
								}
							}
							break;
						default:
				...
				{
					member.name = string.Join(".", path.ToArray());
							...
							switch(c.kind) {
								...
								case CompletionKind.Type:
									if(c.member is Type) {
										...
										if(ReflectionUtils.CanCreateInstance(t)) {
											return new MemberData(ReflectionUtils.CreateInstance(t));
										} else {
											return new MemberData("", t, MemberData.TargetType.Values);
										}
									}
									memberTargetType = MemberData.TargetType.Type;
					...
					while(member.namePath.Length > items.Count) {
						items.Insert(0, null);
					}
