...
namespace MaxyGames.uNode.Nodes {
	[AddComponentMenu(""), EventMenu("GUI", "On Pointer Exit")]
	public class EventOnPointerExit : BaseGraphEvent {
		...
		public override void OnRuntimeInitialize() {
			...
			if(obj != null) {
				if(obj is GameObject) {
					UEvent.Register<PointerEventData>(UEventID.OnPointerExit, obj as GameObject, OnTriggered);
				} else if(obj is Component) {
					UEvent.Register<PointerEventData>(UEventID.OnPointerExit, obj as Component, OnTriggered);
				}
			} else {
				UEvent.Register<PointerEventData>(UEventID.OnPointerExit, owner, OnTriggered);
			}
		}
