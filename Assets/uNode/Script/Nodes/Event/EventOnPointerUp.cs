...
namespace MaxyGames.uNode.Nodes {
	[AddComponentMenu(""), EventMenu("GUI", "On Pointer Up")]
	public class EventOnPointerUp : BaseGraphEvent {
		...
		public override void OnRuntimeInitialize() {
			...
			if(obj != null) {
				if(obj is GameObject) {
					UEvent.Register<PointerEventData>(UEventID.OnPointerUp, obj as GameObject, OnTriggered);
				} else if(obj is Component) {
					UEvent.Register<PointerEventData>(UEventID.OnPointerUp, obj as Component, OnTriggered);
				}
			} else {
				UEvent.Register<PointerEventData>(UEventID.OnPointerUp, owner, OnTriggered);
			}
		}
