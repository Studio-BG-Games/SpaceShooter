...
namespace MaxyGames.uNode.Nodes {
	[AddComponentMenu(""), EventMenu("GUI", "On Pointer Enter")]
	public class EventOnPointerEnter : BaseGraphEvent {
		...
		public override void OnRuntimeInitialize() {
			...
			if(obj != null) {
				if(obj is GameObject) {
					UEvent.Register<PointerEventData>(UEventID.OnPointerEnter, obj as GameObject, OnTriggered);
				} else if(obj is Component) {
					UEvent.Register<PointerEventData>(UEventID.OnPointerEnter, obj as Component, OnTriggered);
				}
			} else {
				UEvent.Register<PointerEventData>(UEventID.OnPointerEnter, owner, OnTriggered);
			}
		}
				...
				if(target.isAssigned) {
					mData.AddCodeForEvent(
						CG.FlowInvoke(
							...
							CG.Value(UEventID.OnPointerEnter),
							CG.Value(target),
