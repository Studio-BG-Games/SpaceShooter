...
namespace MaxyGames.uNode.Nodes {
	[AddComponentMenu(""), EventMenu("GUI", "On Pointer Move")]
	public class EventOnPointerMove : BaseGraphEvent {
		...
		public override void OnRuntimeInitialize() {
			...
			if(obj != null) {
				if(obj is GameObject) {
					UEvent.Register<PointerEventData>(UEventID.OnPointerMove, obj as GameObject, OnTriggered);
				} else if(obj is Component) {
					UEvent.Register<PointerEventData>(UEventID.OnPointerMove, obj as Component, OnTriggered);
				}
			} else {
				UEvent.Register<PointerEventData>(UEventID.OnPointerMove, owner, OnTriggered);
			}
		}
				...
				if(target.isAssigned) {
					mData.AddCodeForEvent(
						CG.FlowInvoke(
							...
							CG.Value(UEventID.OnPointerMove),
							CG.Value(target),
