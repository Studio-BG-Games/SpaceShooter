		...
		protected override void OnExecute() {
			if(gameObject.isAssigned && componentType.isAssigned) {
				if(storeComponent.isAssigned) {
					storeComponent.Set(gameObject.Get<GameObject>().GetComponentInParent(componentType.Get() as System.Type));
					return;
				}
				gameObject.Get<GameObject>().GetComponentInParent(componentType.Get() as System.Type);
			}
		}

		public override string GenerateCode(Object obj) {
			if(gameObject.isAssigned && componentType.isAssigned) {
				string data = CG.Invoke(gameObject, "GetComponentInParent", componentType.CGValue());
