...
namespace VSX.UniversalVehicleCombat
{
    ...
    /// </summary>
    public class HealthSpaceCombat : ModuleManager
    {
        ...
        protected override void OnModuleMounted(Module module)
        {
            ...
            HealthSpaceCombat healthSpaceCombat = module.GetComponent<HealthSpaceCombat>();
            if (healthSpaceCombat == this) return;

            if (healthSpaceCombat != null)
            {
                for (int i = 0; i < healthSpaceCombat.damageables.Count; ++i)
                {
                    damageables.Add(healthSpaceCombat.damageables[i]);
                    if (overrideDamageableDefaultSettings)
                    {
                        healthSpaceCombat.damageables[i].SetDamageable(defaultIsDamageable);
                        healthSpaceCombat.damageables[i].SetHealable(defaultIsHealable);
                    }
                }
            }
            else
        ...
        protected override void OnModuleUnmounted(Module module)
        {
            ...
            if (healthSpaceCombat != null)
            {
                for (int i = 0; i < healthSpaceCombat.damageables.Count; ++i)
                {
                    if (damageables.Contains(healthSpaceCombat.damageables[i]))
                    {
                        damageables.Remove(healthSpaceCombat.damageables[i]);
                    }
                }
            }
            else
